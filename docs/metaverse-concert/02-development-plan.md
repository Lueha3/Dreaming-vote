...
**포함**: 공연장 템플릿 1개(중형 라이브홀) + 버스킹 존(편성제), 사전녹음 업로드 + ACRCloud 자동검수 + 수동 승인, 셋리스트(15곡 제한), 동접 30명 룸, 아바타 + 이모트 6종 + 응원봉(스프링 물리), 위치별 리버브 2존 + 군중 사운드 3레이어, 아티스트 텍스트 멘트·연출 트리거·자동 립싱크, 별점 + SP 원장, 주간 리더보드, 세로 클립 자동 생성.
**제외**: VR, 라이브 마이크, 커스텀 공연장, 현금 결제, 분기 시즌(주간으로 대체), 네이티브 앱(모바일 웹), 군중 물리.
...
동접 100~200(AOI), 응원봉 리듬 판정, 연출 타임라인 에디터 고도화, 흥분도 기반 군중 사운드, 시즌 랭킹 정식화, 어뷰징 방어(참석 검증·리워드 상한·클러스터 탐지), 음성 멘트 + 라이트 QTE. 인프라 월 $200~400.

### 베타 (3개월) — 공개, 수익화 시동
인스턴스 샤딩 + 팬텀 크라우드, 모바일 최적화(웹 유지 또는 Capacitor 래핑 — IAP 시 스토어 수수료 15~30% 감안), 크리스탈 결제(토스페이먼츠), 굿즈·팬클럽 멤버십, 인앱 비트 스튜디오(Mubert) + 보컬 녹음, 라이브 마이크 공연(LiveKit), Unity 클라이언트 착수 여부 결정 게이트. 팀 4~5인 필수 구간: 분기 인건비 약 1~1.5억원 + 인프라 월 $500~1,000.

### 정식 (베타 후 2개월~)
첫 분기 어워드 시즌(요구사항 7), 아티스트 현금 정산 체계(T1 파트너), 표정 캡처 opt-in, VR(WebXR/Quest), 글로벌 리전(도쿄 게임서버 — 일본 파일럿).
