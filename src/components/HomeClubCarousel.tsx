 ...
 * 풀블리드(양옆 peek)·세로형 이미지 카드 + 하단 스크림 위 흰 타이포 락업(이름/메타/카운터).
 * 커버는 동아리 카드뉴스 첫 장(order 0), 없으면 카테고리 딥 그라데이션 폴백.
 * 활성 슬라이드의 accent 색이 상단 배경으로 은은히 번진다(앰비언트) — 스크롤로 중앙 카드를 추적.
 * 최신 개설이 1번(왼쪽), 뒤로 갈수록 과거. 실은 수보다 동아리가 많으면 끝에 '전체 보기' 슬라이드.
 */
export function HomeClubCarousel({ clubs, total }: { clubs: ClubSlide[]; total: number }) {
  ...
  // 중앙에 가장 가까운 슬라이드의 accent를 배경 글로우 CSS 변수로 반영.
  // 리렌더 없이 DOM에 직접 써 스크롤 중 상태 churn을 피한다. rAF로 프레임당 1회.
  useEffect(() => {
        ...
        >
          {clubs.map((c, i) => (
            <Link
              ...
              className="home-slide relative aspect-[4/5] w-[82%] shrink-0 snap-center overflow-hidden rounded-[26px] sm:w-[46%]"
            >
              ...
              <span
                ...
                style={{
                  background:
                    "linear-gradient(180deg,rgba(8,18,28,.12) 0%,rgba(8,18,28,0) 26%,rgba(8,18,28,0) 42%,rgba(8,18,28,.68) 100%)",
                }}
              />
              <span
                aria-hidden
              ...
              {/* NEW 칩 — 가장 최근 1건 */}
              {i === 0 && (
                <span className="absolute left-3.5 top-3.5 rounded-full bg-gradient-to-r from-gold to-teal px-2.5 py-1 text-[10px] font-extrabold tracking-[0.06em] text-[#2B2103]">
                  NEW
                </span>
              )}
              {!c.imageUrl && (
                <span className="absolute right-3.5 top-3.5 rounded-full border border-white/35 bg-[#0F1E2D]/35 px-2 py-0.5 text-[9px] font-semibold text-white/85 backdrop-blur-sm">
                  기본 커버
                </span>
              )}
              ...
              <span className="absolute inset-x-4 bottom-4 text-center">
                <span className="line-clamp-2 block text-[20px] font-extrabold leading-tight tracking-tight text-white [text-shadow:0_2px_14px_rgba(0,0,0,.35)]">
                  {c.name}
                </span>
                <span className="mt-1 block truncate text-[11.5px] font-medium text-white/85 [text-shadow:0_1px_8px_rgba(0,0,0,.3)]">
                  {c.category}
                  {c.keyword ? ` · ${c.keyword}` : ""} · 멤버 {c.memberCount}
                </span>
                <span className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-bold tabular-nums tracking-[0.06em] text-white/75">
                  {pad2(i + 1)}
                  <span className="font-normal opacity-50">|</span>
                  {pad2(shown)}
                  {hasMore && (
                    <span className="grid h-4 w-4 place-items-center rounded-full border border-white/55 text-[11px] font-normal leading-none">
                      +
                    </span>
                  )}
                </span>
          ...
          {hasMore && (
            <Link
              ...
              className="home-slide relative grid aspect-[4/5] w-[82%] shrink-0 snap-center place-items-center overflow-hidden rounded-[26px] border border-white/70 bg-white/55 sm:w-[46%]"
            >
